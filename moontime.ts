const moon = {
  ...
  now: function (t?: number) { return this.settings.smooth ? this.smooth(t) : this.api(t) || this.local(t) },
  local: function (t: number = performance.now()) { return this._time._initialMoon + (t - this._time._initialLocal) / this.const.MICRO_MOMENT_LEN },
  smooth: function (t: number = performance.now()) { return this._time._latestSmooth + (t - this._time._latestSmoothUpdate) / this.const.MICRO_MOMENT_LEN },
  api: function (t: number = performance.now()) { return this._time._latestAPI === -1 ? undefined : this._time._latestAPI + (t - this._time._latestAPIUpdate) / this.const.MICRO_MOMENT_LEN },

  solarToMoon: function (milliseconds: number) { return (milliseconds - this.const.EPOCH) / this.const.MICRO_MOMENT_LEN },
  moonToSolar: function (micromoments: number) { return micromoments * this.const.MICRO_MOMENT_LEN + this.const.EPOCH },

  init: function (mode: 'manual' | 'auto' = 'manual'): false | void {
    const scripts = document.querySelectorAll('script')
    const script = Array.from(scripts).find(s => s.src.indexOf('moontime.js') > -1 && s.getAttribute('delay_start') !== null)
    if (script && mode === 'auto') return false
  ...
  _updateSmooth: function () {
    ...
    if (this._time._latestAPI !== -1) {
      ...
      else this._time._latestSmooth = this._time._latestSmooth + (this._time._latestSmoothUpdate - prevSmoothUpdate) / this.const.MICRO_MOMENT_LEN + offset / this.settings.smoothFactor
    } else {
      this._time._latestSmoothUpdate = performance.now()
      this._time._latestSmooth = this.local(this._time._latestSmoothUpdate)
    }
  ...
  _fetchAPI: async function () {
    try {
      ...
      if (time === undefined || isNaN(time) || relativeMoon === undefined || isNaN(relativeMoon)) throw new Error('Invalid response: ' + res)
  ...
  formatMoonTime: function (micromoments?: number) {
  ...
  formatMoonString: function (string: string, time?: number) {
